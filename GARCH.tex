...
\begin{align*}
	E[\epsilon_t\ |\ \psi_{t-1}] &= \mu < \infty, \\
	V[\epsilon_t\ |\ \psi_{t-1}] &= \sigma^2 < \infty, \\
	Cov[\epsilon_t,\ \epsilon_{t-i}\ |\ \psi_{t-1}] &= \rho_i < \infty.
\end{align*}
と表せるための必要十分条件は
\[
...
を仮定すれば，
\[
	E[\epsilon_t\ |\ \psi_{t-1}] = 0,\ V[\epsilon_t\ |\ \psi_{t-1}] = \alpha_0 \left( 1 - \sum_{i=1}^{q} \alpha_i - \sum_{i=1}^{p} \beta_i \right)^{-1},
	\ Cov[\epsilon_t,\ \epsilon_s\ |\ \psi_{t-1}] &= 0
\]
が成り立つ．特に，$GARCH(1,\ 1)$では
\[
	E[\epsilon_t\ |\ \psi_{t-1}] = 0,\ V[\epsilon_t\ |\ \psi_{t-1}] = \alpha_0 \left( 1 - \alpha_1 - \beta_1 \right)^{-1},
	\ Cov[\epsilon_t,\ \epsilon_s\ |\ \psi_{t-1}] &= 0 \Leftrightarrow \alpha_1 + \beta_1 < 1
\]
である．
}
